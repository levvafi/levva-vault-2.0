...
///@dev forge script script/release/arbitrum-test/03.DeployUSDCVault.s.sol:DeployUSDCVault -vvvv --account testDeployer --rpc-url $ARB_RPC_URL --verify --etherscan-api-key  $ETHERSCAN_KEY --broadcast
contract DeployUSDCVault is LevvaVaultDeployer {
    ...
    function _getDeployConfig() internal view override returns (VaultConfig[] memory configs) {
        if (block.chainid == 42161) {
            ...
            adapters[4] = Adapter.MorphoV1_1;
            adapters[5] = Adapter.PendleAdapter;
            adapters[6] = Adapter.UniswapAdapter;

            VaultConfig memory config = VaultConfig({
                deploymentId: "LEVVA-USDC-TEST",
